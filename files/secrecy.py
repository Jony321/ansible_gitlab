...
def check_file(ctx: Context, content: bytes, path: str):
    ...
    check_hashed_password(ctx, content, path)
...
def check_hashed_password(ctx: Context, content: bytes, path: str):
    pattern = re.compile(b"[$](1|2a|2y|5|6)[$].*[$].*")
    for lineno, line in enumerate(content.splitlines(), start=1):
        if pattern.search(line) is not None:
            linestr = line.decode(errors="replace")
            ctx.line_error(path, lineno, f'looks like hashed password: {linestr}')
